export default class TemplateBuilder {
    ...
    setTitle(title = ``) {
        this.template += `<p class="text-5xl">${title}</p>`;
    }
    ...
    addAnswer(answer) {
        this.template += `<div class="answer-block mt-8 text-5xl">`;
        if(answer.text !== null) {
    ...
    addAnswerButton() {
        this.template += `</div><br><button class="btn btn-success btn-lg text-5xl" id="get_next">Ответить и перейти дальше</button>`;
